...
SELECT SUM(surface_area) AS "アジア大陸の表面積" FROM countries
WHERE continent = 'Asia';

-- 問25
-- 全ての国と言語を表示してください。一つの国に複数言語があると思いますので同じ国名を言語数だけ出力してください。
SELECT name, language FROM countrylanguages
...
INNER JOIN countries ON countrylanguages.country_code = code
INNER JOIN cities ON cities.country_code = countrylanguages.country_code;

-- 問27
-- 全ての有名人を出力してください。左側外部結合を使用して国名なし（country_codeがNULL）も表示してください。
SELECT celebrities.name, countries.name
FROM celebrities
LEFT JOIN countries ON code = celebrities.country_code;

-- 問28
-- 全ての有名人の名前,国名、第一言語を出力してください。
SELECT celebrities.name, countries.name, countrylanguages.language
FROM celebrities
LEFT JOIN countries ON celebrities.country_code = code
LEFT JOIN countrylanguages ON code = countrylanguages.country_code
...
SELECT celebrities.country_code, MAX(age), MIN(age)
FROM celebrities
INNER JOIN countries ON countries.code = celebrities.country_code
